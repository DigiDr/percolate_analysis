...
int percolate(int length, float rho, int seed, int maxClusters, char* datFile, char* pgmFile);
...
int main(int argc, char* argv[])
{
  ...
  percolate(length, rho, seed, maxClusters, datFile, pgmFile);
}
...

{
  ...
  int while_loop, outer_for_loop, inner_for_loop, nchange, old;
  while_loop = 1;
  outer_for_loop = 1;
  inner_for_loop = 1;
  nchange = 1;
  while (nchange > 0)
    ...
    outer_for_loop++;
      ...
      printf("Number of iterations of: while loop is %d, outer for loop (i) is %d, inner for loop (j) is %d and total number of changes is %d\n", while_loop, outer_for_loop, inner_for_loop, nchange);
      while_loop++;
    }
  ...
  printf("Opening loop file \n");
  char loop_string[100];
  sprintf(loop_string, "%d_%d_loops.txt", length, seed);

  FILE *fp;
  fp = fopen(loop_string, "w");
  printf("Writing loop ...\n");

  fprintf(fp, "%d\n", while_loop);
  fprintf(fp, "%d", inner_for_loop);

  printf("...done\n");
  fclose(fp);
  printf("Closed loop file \n");
